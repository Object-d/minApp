      ...
      <nut-form class="form-container" :model-value="dynamicForm.state" ref="dynamicRefForm">
        ...
        <template v-for="(i, index) in dynamicForm.state.followVisitors" :key="i.key">
          ...
          <nut-form-item label="手机号" :prop="'followVisitors.' + index + '.v_phone'" required
                         :rules="[{ required: true, message: '请输入手机号' }, { pattern: /^1[3|4|5|7|8|9][0-9]{9}$/, message: '请输入格式正确的手机号' }]">
          ...
          <nut-form-item :label-width="'calc(100vw - 143px)'">
            ...
            <nut-uploader :disabled="!canEdit" url="http://服务地址" :auto-upload="false" accept="image/*"
                          v-model:file-list="i.faceFile" maximum="1" multiple :before-upload="dynamicForm.methods.beforeUpload">
            </nut-uploader>
          </nut-form-item>
        </template>
...
const dynamicForm: any = {
  ...
  methods: {
    async beforeUpload(files: File[]) {
      const fileToDataURL = (file: any): Promise<any> => {
      ...
      // @ts-ignore
      dynamicForm.fileBase64Map[files[0].path] = base64
      return files
    },
    submit() {
      dynamicRefForm.value.validate().then(async ({valid, errors}) => {
        if (valid) {
          const state = dynamicForm.state
          const fileBase64Map = dynamicForm.fileBase64Map
          const followVisitors = state.followVisitors
          const fv = followVisitors[0]
          const data = {
            isv_id: state.isv_id.value,
            v_reason: state.v_reason,
            memo: state.memo || undefined,
            v_timeStr: state.v_timeStr?.replaceAll('/', '-'),
            v_lvTimeStr: state.v_lvTimeStr?.replaceAll('/', '-'),
            ...
            v_dw: fv.v_dw,
            v_name: fv.v_name,
            v_certificateType: 111,
            v_personSum: followVisitors.length,
            v_certificateNumber: fv.v_certificateNumber,
            v_plateNumber: fv.v_plateNumber || undefined,
            faceFile: fileBase64Map[fv?.faceFile?.[0]?.path],
            followVisitors: followVisitors.slice(1).map(t => ({
              v_phone: t.v_phone,
              v_name: t.v_name,
              v_certificateType: 111,
              v_certificateNumber: t.v_certificateNumber,
              v_plateNumber: t.v_plateNumber || undefined,
...
const visitWhoPicker = reactive({
  async request(searchKey) {
    const res = await request.post('/evo-apigw/evo-brm/1.2.0/person/subsystem/page', {
      pageSize: 50,
      pageNum: 1,
      statusList: [1],
      searchKey
    ...
    if (res.success) {
      return Promise.resolve({
        data: res.data.pageData.map(t => ({ ...t, text: t.name, value: t.id }))
      })
    }
  },
  onShow() {
